(ns rclojure.core.fn-expression
  (:require [rclojure.core.coll-expression :refer [as-vec str-combine]])
...
(defn cat
  ...
  ([coll {:keys [file sep fill labels]}]
   (cond (and coll file sep (instance? Boolean fill) labels)
         (str "cat("coll", file = \""file"\", sep = \""sep"\", fill = "(.toUpperCase (str fill))", labels = " (if (nil? labels) "NULL"(as-vec  labels {:mode :character}))")")
...
(defn matrix
  ...
  ([coll {:keys [nrow ncol byrow rows cols]}]
   (cond (and coll nrow ncol (instance? Boolean byrow) rows cols)
         (str "matrix("coll", nrow = "nrow", ncol = "ncol", byrow = "(.toUpperCase (str byrow))", dimnames = list("(as-vec rows {:mode :character})","(as-vec cols {:mode :character})"))")
         (and coll nrow ncol (instance? Boolean byrow) rows)
         (str "matrix("coll", nrow = "nrow", ncol = "ncol", byrow = "(.toUpperCase (str byrow))", dimnames = list("(as-vec rows {:mode :character})"))")
...
(defn plot-vec ([coll] (str "plot("coll")")))
